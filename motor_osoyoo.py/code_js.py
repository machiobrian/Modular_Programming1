...
pygame.init()
controller = pygame.joystick.Joystick(0) #select the first joystick as in opencv
controller.init()

#use a dictionary by the name buttons
...
def getJS(name=''):
    ...
    for event in pygame.event.get():            #Analog sticks
        ...
        elif event.type == pygame.JOYBUTTONDOWN: #when the button is pressed
            for x,(key,val) in enumerate(buttons.items()):
                if x<10:
                    if controller.get_button(x): buttons[key]=1
        elif event.type == pygame.JOYBUTTONUP:          #when the button is released
            for x,(key,val) in enumerate(buttons.items()):
                if x<10:
